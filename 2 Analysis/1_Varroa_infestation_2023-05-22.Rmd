...
```{r}
m.phor_day0 <- glm.nb(mites_recov_3_washes ~ 
                     trt + 
                     offset(log(est_bee_num_by_wt_floor)),
                   data = tempdatum, subset = days == 0)
summary(aov(m.phor_day0))

# On day 0, no significant differences between treatments (F 2,38 = 0.165, P=0.849)

m.phor_day21 <- glm.nb(mites_recov_3_washes ~ 
                     trt + 
                     offset(log(est_bee_num_by_wt_floor)),
                   data = tempdatum, subset = days == 21)
summary(aov(m.phor_day21))
# On day 21, yes significant differences between treatments (F 2,38 = 6.674, P= 0.00328)

m.phor_day42 <- glm.nb(mites_recov_3_washes ~ 
                     trt + 
                     offset(log(est_bee_num_by_wt_floor)),
                   data = tempdatum, subset = days == 42)
summary(aov(m.phor_day42))

# On day 42, no significant differences between treatments (F 2,34= 1.723, P= 0.194)
...
### ANOVA of infest within day

```{r}
joint_tests(m.infest.f) 
# Does not give a duplicate row names error

# By doing this within days, this may be the ANOVA F test that the reviewer asks for
...
```{r}
m.infest_day0 <- glm.nb(infest_200 ~ 
                     trt,
                   data = tempdatum, subset = days == 0)
summary(aov(m.infest_day0))

# On day 0, no significant differences between treatments (F 2,38= 0.001, P= 0.999)

m.infest_day21 <- glm.nb(infest_200 ~ 
                     trt,
                   data = tempdatum, subset = days == 21)
summary(aov(m.infest_day21))

# On day 21, yes significant differences between treatments (F 2,29= 6.348, P= 0.00517)

m.infest_day42 <- glm.nb(infest_200 ~ 
                     trt,
                   data = tempdatum, subset = days == 42)
summary(aov(m.infest_day42))

# On day 42, no significant differences between treatments (F 2,34= 0.353, P= 0.705)
...
# Added to test idea that the emmeans is using asymptotic degrees of freedom
emmeans(m.phor.f, pairwise ~ trt | days, type = "response")$contrasts
emmeans(m.phor.f, pairwise ~ trt | days, type = "response", lmer.df = "asymptotic")$contrasts
# Yes, same results

# Does Satterthwaite method work? Nope, defaults to asymptotic
emmeans(m.phor.f, pairwise ~ trt | days, type = "response")$contrasts
emmeans(m.phor.f, pairwise ~ trt | days, type = "response", lmer.df = "satterthwaite")$contrasts


# Phor within treatment
emmeans(m.phor.f, pairwise ~ days | trt, type = "response")$contrasts
confint(emmeans(m.phor.f, pairwise ~ days | trt, type = "response"))$contrasts
...
# Infest within treatment
emmeans(m.infest.f, pairwise ~ days | trt, type = "response")$contrasts
confint(emmeans(m.infest.f, pairwise ~ days | trt, type = "response"))$contrasts
